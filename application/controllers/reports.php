...
{	
    ...
    function detailed_requisition($start_date, $end_date , $export_excel=0)
    {
        ...
        foreach($report_data['summary'] as $key=>$row)
        {
            $summary_data[] = array(anchor('requisition/receipt/'.$row['requisition_id'], 'REQS '.$row['requisition_id'], array('target' => '_blank')), $row['requisition_date'], $row['employee_name'], $row['comment']);
